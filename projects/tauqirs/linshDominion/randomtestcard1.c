...
int main() {
    ...
    for (i = 1; i < 2000; i++) {
		...
		G.hand[p][position] = smithy;
		memcpy(&testG, &G, sizeof(struct gameState));
		r = cardEffect(smithy, 0,0,0,&G, 0,0);
		assert(r==0);
		
		printf("hand count = %d, expected = %d\n", G.handCount[p], testG.handCount[p] + 3);
		printf("deck count = %d, expected = %d\n", G.deckCount[p], testG.deckCount[p] - 3);
		if ((G.handCount[p] != testG.handCount[p]+3) && (G.handCount[p] != testG.handCount[p]-3)){
